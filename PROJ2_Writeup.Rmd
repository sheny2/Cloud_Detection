...
```{r pariwise covariate, fig.cap="\\label{fig:corrplot} Pair-wise correlations between eight available features.", out.width="50%"}
All_Image |>
...
```{r, out.width="50%",message=F,warning=F,cache=T}
# ROC for GLM cut
...
t = pROC::coords(roc(Image_Test_Cut$Cloud, GLMpreds), "best", transpose = FALSE)
# GLMpreds <- predict(glm_result, Image_Test_Cut, type = "response")
...
t = pROC::coords(roc(Image_Test_Cut$Cloud01, pred), "best", transpose = FALSE)
invisible(roc(Image_Test_Cut$Cloud01, pred, print.auc = T, plot = T, print.thres = t[[1]]))
...
t = pROC::coords(roc(Image_Test_KM$Cloud, preds), "best", transpose = FALSE)
...
t = pROC::coords(roc(Image_Test_KM$Cloud01, pred), "best", transpose = FALSE)
invisible(roc(Image_Test_KM$Cloud01, pred, print.auc = T, plot = T, print.thres = t[[1]]))
...
```{r prediction plot, fig.cap="\\label{fig:pred_plot} Prediction of cloud pixels using boosting trees", fig.width = 12, fig.height = 4, out.width="85%"}
# prediction plot
...
map1 <- Image_all_KM %>%
  ...
  geom_point(data=filter(Image_all_KM %>%
  filter(image == "1"),Cloud01!=all_prediction),aes(x = X_coord, y = Y_coord), color="red",size = 0.5) +
...
\begin{description}
\item[Step 1.] Utilize Pretrained XGBoost Classifier on the desired data set to get the classification probability $p_i$ for each data point $i$.
...
\item[Step 3.] For the remaining data points $j$, where $LB<p_j<LB$, utilize KNN on geographical coordinates ($X,Y$ in this case) to classify point $j$ to the class of its cloest neighbours.
\end{description}
