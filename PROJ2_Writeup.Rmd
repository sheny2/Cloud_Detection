...
```{r importance plot, fig.cap="\\label{fig:importance_plot}The feature importance plot in boosting trees.", out.width="100%", fig.height=3,fig.width=9}
par(mfrow = c(1, 2)) 
...
xgb.plot.importance(xgb.importance(colnames(as.matrix(Image_Train_Cut[, 4:11])), model = boost_model), main = "Boosting trees on horizontal cut data", cex.main = 0.5, cex.lab = 0.5, cex.axis = 0.5)
...
xgb.plot.importance(xgb.importance(colnames(as.matrix(Image_Train_KM[, 4:11])), model = boost_model), main = "Boosting trees on K-means data", cex.main = 0.5, cex.lab = 0.5, cex.axis = 0.5)
```
...
grid.arrange(arrangeGrob(C1 + ggeasy::easy_center_title() + theme(legend.position="none"),
                         ...
                         nrow = 1),
             C_legend, nrow = 2, heights = c(10, 1))
```
