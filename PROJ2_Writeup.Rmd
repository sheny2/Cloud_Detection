...
```{r pariwise covariate, fig.cap="\\label{fig:corrplot} Pair-wise correlation between available features.", out.width="50%"}
All_Image |>
select(-Y_coord, -X_coord, -Cloud) |>
cor() |>
...
```{r hist covariate, fig.cap="\\label{fig:hist_cov}Density distributions of three features that describe cloud and clear pixels.", fig.width=9, fig.height=3, out.width="85%"}
hist_1 <- All_Image %>% 
  ggplot(aes(x = NDAI )) +
...
```{r}
all_result <- read.csv("all_result.csv") %>% mutate(Cv.mean = CV.Average, Test = Test.Accuracy) %>% select(-CV.Average, -Test.Accuracy)

# all_result %>% select(1:2, fold.1) %>% tidyr::pivot_wider(names_from = classifier:data, values_from = fold.1)
# all_result %>% select(1:2, fold.2) %>% tidyr::pivot_wider(names_from = classifier:data, values_from = fold.2)
# all_result %>% select(1:2, fold.3) %>% tidyr::pivot_wider(names_from = classifier:data, values_from = fold.3)
# all_result %>% select(1:2, fold.4) %>% tidyr::pivot_wider(names_from = classifier:data, values_from = fold.4)
# all_result %>% select(1:2, fold.5) %>% tidyr::pivot_wider(names_from = classifier:data, values_from = fold.5)
# all_result %>% select(1:2, fold.6) %>% tidyr::pivot_wider(names_from = classifier:data, values_from = fold.6)
# all_result %>% select(1:2, fold.7) %>% tidyr::pivot_wider(names_from = classifier:data, values_from = fold.7)
