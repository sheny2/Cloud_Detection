...
probt <- predict(glm_test, Image_Train_Cut, type = "response")
t = pROC::coords(pROC::roc(Image_Train_Cut$Cloud, probt), "best", transpose = FALSE)

prob <- predict(glm_test, Image_Test_Cut, type = "response")
prediction <- ifelse(prob > t[[1]], 1,-1)
...
probt <- predict(glm_test, Image_Train_KM, type = "response")
t = pROC::coords(pROC::roc(Image_Train_KM$Cloud, probt), "best", transpose = FALSE)
...
xgb.plot.tree(model = final_boost_All, trees = 1000, plot_width = 1000, plot_height = 1000)
