...
```{r 5 groups, fig.width=12, fig.height=3}
...
grid.arrange(Cut_1_Map, Cut_2_Map, Cut_3_Map, nrow = 1)
...
```{r 5 KNN groups, fig.width=12, fig.height=3}
set.seed(8848)
...
KM_1_Map <- Image_1 %>%
  ggplot() + geom_point(aes(x = X_coord, y = Y_coord, color = factor(KM_1$cluster)), size = 0.5) +
  theme_bw() + labs(title = "Image 1", x = "X Coordinate", y = "Y Coordinate") +  scale_color_manual(values = c("red", "blue", "green", "green", "green"))  

KM_2_Map <- Image_2 %>%
  ggplot() + geom_point(aes(x = X_coord, y = Y_coord, color = factor(KM_2$cluster)), size = 0.5) +
  theme_bw() + labs(title = "Image 2", x = "X Coordinate", y = "Y Coordinate") +  scale_color_manual(values = c("red", "blue", "green", "green", "green"))  

KM_3_Map <- Image_3 %>%
  ggplot() + geom_point(aes(x = X_coord, y = Y_coord, color = factor(KM_3$cluster)), size = 0.5) +
  theme_bw() + labs(title = "Image 3", x = "X Coordinate", y = "Y Coordinate") +  scale_color_manual(values = c("red", "blue", "green", "green", "green"))  

grid.arrange(KM_1_Map, KM_2_Map, KM_3_Map, nrow = 1)

Image_all_KM <- rbind(Image_1 %>% mutate(cluster = KM_1$cluster),
                      Image_2 %>% mutate(cluster = KM_2$cluster),
                      Image_3 %>% mutate(cluster = KM_3$cluster))

Image_all_KM <- Image_all_KM %>% 
  mutate(Type = ifelse(cluster == "1", "Test", "Train"))  %>% 
  mutate(Type = ifelse(cluster == "2", "Validation", Type)) 
...
```{r}
CV_Master <- function(Classifier, train_coord, train_feature, train_label, K, loss_fun) {
  ...
  for (i in 1:K){
   ...
   glm_result[[i]] = glm(paste("train_label ~ ", paste(names(train_feature), collapse = "+" ), sep = "" ), data = dat_CV_train, family = 'binomial')
